...
func WriteCanonicalHash(db aquadb.Putter, hash common.Hash, number uint64) error {
...
func WriteHeadHeaderHash(db aquadb.Putter, hash common.Hash) error {
...
func WriteHeadBlockHash(db aquadb.Putter, hash common.Hash) error {
...
func WriteHeadFastBlockHash(db aquadb.Putter, hash common.Hash) error {
...
func WriteTrieSyncProgress(db aquadb.Putter, count uint64) error {
...
func WriteHeader(db aquadb.Putter, header *types.Header) error {
...
func WriteBody(db aquadb.Putter, hash common.Hash, number uint64, body *types.Body) error {
...
func WriteBodyRLP(db aquadb.Putter, hash common.Hash, number uint64, rlp rlp.RawValue) error {
...
func WriteTd(db aquadb.Putter, hash common.Hash, number uint64, td *big.Int) error {
...
func WriteBlock(db aquadb.Putter, block *types.Block) error {
...
func WriteBlockReceipts(db aquadb.Putter, hash common.Hash, number uint64, receipts types.Receipts) error {
...
func WriteTxLookupEntries(db aquadb.Putter, block *types.Block) error {
...
func WriteBloomBits(db aquadb.Putter, bit uint, section uint64, head common.Hash, bits []byte) {
	key := append(append(bloomBitsPrefix, make([]byte, 10)...), head.Bytes()...)
...
func PreimageTable(db aquadb.Database) aquadb.Database {
	return aquadb.NewTable(db, preimagePrefix)
}

// WritePreimages writes the provided set of preimages to the database. `number` is the
// current block number, and is used for debug messages only.
func WritePreimages(db aquadb.Database, number uint64, preimages map[common.Hash][]byte) error {
...
func WriteBlockChainVersion(db aquadb.Putter, vsn int) {
	enc, _ := rlp.EncodeToBytes(uint(vsn))
	db.Put([]byte("BlockchainVersion"), enc)
}

// WriteChainConfig writes the chain config settings to the database.
func WriteChainConfig(db aquadb.Putter, hash common.Hash, cfg *params.ChainConfig) error {
...
func FindCommonAncestor(db DatabaseReader, a, b *types.Header, versionFunc func(*big.Int) params.HeaderVersion) *types.Header {
	...
	for a.SetVersion(byte(versionFunc(a.Number))) != b.SetVersion(byte(versionFunc(b.Number))) {
		a = GetHeaderNoVersion(db, a.ParentHash, a.Number.Uint64()-1)
		if a == nil {
			return nil
		}
