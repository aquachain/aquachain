...
var (
	...
	monitorCommand = cli.Command{
		...
		Description: `
The AquaChain monitor is a tool to collect and visualize various internal metrics
...
func monitor(ctx *cli.Context) error {
	...
	if len(monitored) == 0 {
		...
		if len(list) > 0 {
			...
			utils.Fatalf("No metrics collected by aquachain (--%s).\n", utils.MetricsEnabledFlag.Name)
		}
	}
...
// retrieveMetrics contacts the attached aquachain node and retrieves the entire set
