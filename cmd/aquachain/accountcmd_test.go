...
`)
	aquachain.ExpectRegexp(`Address: \{0x[0-9a-f]{40}\}\n`)
}
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(aquachain.Datadir, "keystore"))
	if len(files) != 1 {
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(aquachain.StderrText(), m) {
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	aquachain.SetTemplateFunc("keypath", func(file string) string {
