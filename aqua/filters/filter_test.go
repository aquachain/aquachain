...
func BenchmarkFilters(b *testing.B) {
	...
	var (
		db, _      = aquadb.NewLDBDatabase(dir, 0, 0)
	...
	chain, receipts := core.GenerateChain(context.TODO(), params.TestChainConfig, genesis, aquahash.NewFaker(), db, 100010, func(i int, gen *core.BlockGen) {
		switch i {
		case 2403:
...
func TestFilters(t *testing.T) {
	...
	chain, receipts := core.GenerateChain(context.TODO(), params.TestChainConfig, genesis, aquahash.NewFaker(), db, 1000, func(i int, gen *core.BlockGen) {
		switch i {
		case 1:
			receipt := types.NewReceipt(nil, false, 0)
			receipt.Logs = []*types.Log{
