...
// Package filters implements an aquachain filtering system for block,
...
type subscription struct {
	...
	typ       Type
	created   time.Time
	logsCrit  aquachain.FilterQuery
	logs      chan []*types.Log
...
func (es *EventSystem) SubscribeLogs(crit aquachain.FilterQuery, logs chan []*types.Log) (*Subscription, error) {
...
func (es *EventSystem) subscribeMinedPendingLogs(crit aquachain.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) subscribeLogs(crit aquachain.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) subscribePendingLogs(crit aquachain.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) lightFilterNewHead(newHeader *types.Header, callBack func(*types.Header, bool)) {
	...
	for oldh.Hash() != newh.Hash() {
		if oldh.Number.Uint64() >= newh.Number.Uint64() {
			...
			oldh = core.GetHeaderNoVersion(es.backend.ChainDb(), oldh.ParentHash, oldh.Number.Uint64()-1)
			oldh.Version = es.backend.GetHeaderVersion(oldh.Number)
		}
		if oldh.Number.Uint64() < newh.Number.Uint64() {
			newHeaders = append(newHeaders, newh)
			newh = core.GetHeaderNoVersion(es.backend.ChainDb(), newh.ParentHash, newh.Number.Uint64()-1)
			if newh == nil {
				// happens when CHT syncing, nothing to do
				newh = oldh
			} else {
				newh.Version = es.backend.GetHeaderVersion(newh.Number)
			}
		}
	}
