...
import (
	...
	"gitlab.com/aquachain/aquachain/aqua/accounts/usbwallet/internal/trezor"
...
// or whether the Aquachain app was not started on it.
...
// Aquachain address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.AquachainAddress)
	if _, err := w.trezorExchange(&trezor.AquachainGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	request := &trezor.AquachainSignTx{
	...
	response := new(trezor.AquachainTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.AquachainTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
