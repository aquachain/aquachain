...
type fetchRequest struct {
	...
	From    uint64          // [aqua/62] Requested chain element index (used for skeleton fills only)
	Headers []*types.Header // [aqua/62] Requested headers, sorted by request order
...
type queue struct {
	...
	blockTaskQueue *prque.Prque                  // [aqua/62] Priority queue of the headers to fetch the blocks (bodies) for
	blockPendPool  map[string]*fetchRequest      // [aqua/62] Currently pending block (body) retrieval operations
	blockDonePool  map[common.Hash]struct{}      // [aqua/62] Set of the completed block (body) fetches

	receiptTaskPool  map[common.Hash]*types.Header // [aqua/63] Pending receipt retrieval tasks, mapping hashes to headers
	receiptTaskQueue *prque.Prque                  // [aqua/63] Priority queue of the headers to fetch the receipts for
	receiptPendPool  map[string]*fetchRequest      // [aqua/63] Currently pending receipt retrieval operations
	receiptDonePool  map[common.Hash]struct{}      // [aqua/63] Set of the completed receipt fetches
...
func (q *queue) Schedule(headers []*types.Header, from uint64) []*types.Header {
	...
	for _, header := range headers {
		...
		if header.Number.Uint64() == 22801 && q.headerHead != (common.Hash{}) && q.headerHead != header.ParentHash {
			log.Trace("Undead chain detected, header breaks chain ancestry", "number", header.Number, "hash", hash)
			break
		}
		if q.headerHead != (common.Hash{}) && q.headerHead != header.ParentHash {
			log.Warn("Incoming header broke chain ancestry, skipping rest of chain", "number", header.Number, "hash", hash)
			break
		}
