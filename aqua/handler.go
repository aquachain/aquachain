...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	case msg.Code == GetBlockHeadersMsg:
		...
		p.Log().Warn("Sending headers", "peer", p.ID().String()[:6], "headers", len(headers))
	...
	case msg.Code == BlockHeadersMsg:
		...
		p.Log().Warn("Recv headers", "peer", p.ID().String()[:6], "headers", len(headers))
...
type NodeInfo struct {
	Network    uint64              `json:"network"`    // AquaChain network ID (1=Frontier, 2=Morden, Ropsten=3, Rinkeby=4)
