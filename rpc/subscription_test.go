...
func TestNotifications(t *testing.T) {
	...
	if _, err := server.RegisterName("aqua", service); err != nil {
		t.Fatalf("unable to register test service %v", err)
	}

	clientConn, serverConn := net.Pipe()

	go server.ServeCodec("test", NewJSONCodec(serverConn), OptionMethodInvocation|OptionSubscriptions)
...
func TestSubscriptionMultipleNamespaces(t *testing.T) {
	var (
		namespaces             = []string{"aqua", "shh", "bzz"}
		server                 = NewServer()
		service                = NotificationTestService{}
		clientConn, serverConn = net.Pipe()
	...
	for _, namespace := range namespaces {
		if _, err := server.RegisterName(namespace, &service); err != nil {
			t.Fatalf("unable to register test service %v", err)
		}
	}
