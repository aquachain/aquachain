...
func newWorker(config *params.ChainConfig, engine consensus.Engine, coinbase common.Address, aqua Backend, mux *event.TypeMux) *worker {
	worker := &worker{
		config:         config,
		engine:         engine,
		aqua:           aqua,
		...
		chainDb:        aqua.ChainDb(),
		recv:           make(chan *Result, resultQueueSize),
		chain:          aqua.BlockChain(),
		proc:           aqua.BlockChain().Validator(),
		possibleUncles: make(map[common.Hash]*types.Block),
		coinbase:       coinbase,
		agents:         make(map[Agent]struct{}),
		unconfirmed:    newUnconfirmedBlocks(aqua.BlockChain(), miningLogAtDepth),
	}
	// Subscribe TxPreEvent for tx pool
	worker.txSub = aqua.TxPool().SubscribeTxPreEvent(worker.txCh)
	// Subscribe events for blockchain
	worker.chainHeadSub = aqua.BlockChain().SubscribeChainHeadEvent(worker.chainHeadCh)
	worker.chainSideSub = aqua.BlockChain().SubscribeChainSideEvent(worker.chainSideCh)
	go worker.update()
...
func (w *worker) setAquabase(addr common.Address) {
	w.mu.Lock()
	defer w.mu.Unlock()
	w.coinbase = addr
}
...
func (w *worker) makeCurrent(parent *types.Block, header *types.Header) error {
	...
	for _, ancestor := range w.chain.GetBlocksFromHash(parent.Hash(), 7) {
		for _, uncle := range ancestor.Uncles() {
			work.family.Add(uncle.SetVersion(byte(w.config.GetBlockVersion(uncle.Number))))
		}
...
func (w *worker) commitNewWork() {
	...
	for hash, uncle := range w.possibleUncles {
		...
		unclehead.Version = w.chain.Config().GetBlockVersion(unclehead.Number)
		if err := w.commitUncle(work, unclehead); err != nil {
			log.Trace("Bad uncle found and will be removed", "hash", hash, "reason", err)
			log.Trace(uncle.String())
	...
	// compute tx for new block
	pending, err := w.aqua.TxPool().Pending()
	if err != nil {
...
func (env *Work) commitTransactions(mux *event.TypeMux, txs *types.TransactionsByPriceAndNonce, bc *core.BlockChain, coinbase common.Address) {
	...
	for {
		...
		if tx.Protected() && !env.config.IsEIP155(env.header.Number) {
			log.Trace("Ignoring replay protected transaction", "hash", tx.Hash(), "eip155", env.config.EIP155Block)
			txs.Pop()
			continue
		}
		...
		case core.ErrGasLimitReached:
			...
			log.Trace("Fuel limit exceeded for current block", "sender", from)
			txs.Pop()
		...
		case core.ErrNonceTooHigh:
			...
			log.Trace("Skipping account, nonce too high", "sender", from, "nonce", tx.Nonce())
			txs.Pop()
