...
func (c *ChainConfig) NextHF(cur *big.Int) *big.Int {
	if cur == nil {
		return AquachainHF[0]
	}
	if cur != nil {
		for hf := 0; hf < len(AquachainHF); hf++ {
			if height := AquachainHF[hf]; height != nil && cur.Cmp(height) < 0 {
				return height
			}
		}
	}
